...
さらに、devtoolsミドルウェアは他のミドルウェアの後、つまり可能な限り最後に適用すべきです。例えば、immerというミドルウェアを使用する際は、`immer(devtools(...))`の順序が推奨され、`devtools(immer(...))`の順序は避けるべきです。その理由は、devtoolsが`setState`の挙動を変更し、型パラメーターを追加するためで、これがimmerなどの他のミドルウェアがdevtoolsの前に`setState`を変更すると失われてしまう可能性があるからです。そのため、devtoolsを最後に適用することで、その影響を避けることができます。
### **スライスパターン**
